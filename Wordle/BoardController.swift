...
class BoardController: NSObject,
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
    guard let wordLength = settings[kNumLettersKey] as? Int else {
          return
      }
    self.numItemsPerRow = wordLength
  }
  ...
  private func applyNumGuessesSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      guard let guesses = settings[kNumGuessesKey] as? Int else {
            return
        }
      self.numRows = guesses
    // END YOUR CODE HERE
  }
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
     ...
     let wordTheme = WordTheme(rawValue: theme) ?? .normal
      
      let word = WordGenerator.generateGoalWord(with: wordTheme)
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      guard let alien = settings[kIsAlienWordleKey] as? Bool else {
          return
      }
